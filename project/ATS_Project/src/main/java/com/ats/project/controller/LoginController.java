...
public class LoginController {
    ...
    @PostMapping("/login")
    public String loginProc(@ModelAttribute UserVO vo, HttpSession session,
    ...
    public String saveTrustCookie(HttpSession session, HttpServletResponse response) {
        ...
        verifiedCookie.setMaxAge(60 * 60 * 24 * 90); // 90일
    ...
    // ✅ 세션만 설정 (redirect 없이) — verify_done.jsp용
    ...
    @ResponseBody
    public boolean updateMyInfo(@RequestParam String email, @RequestParam String phone,
                                HttpSession session) {
        String userId = (String) session.getAttribute("userId");
        if (userId == null) return false;
        UserVO vo = new UserVO();
    ...
    @ResponseBody
    public Map<String, Object> verifyIdentity(@RequestParam String userId,
    ...
    @ResponseBody
    public Map<String, Object> resetPasswordByEmail(@RequestParam String userId,
                                                     @RequestParam String email,
